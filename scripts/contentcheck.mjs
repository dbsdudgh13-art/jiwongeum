...
const BANNED = [
  // '원본에 세 가지 조건이 ... 명시되어' 처럼 사이에 말이 끼면 못 잡던 구멍이 있었다.
  { re: /원본(?!\s*공고)\s*(자료|정보|데이터)?\s*(에는|에|은|는|이|가)\s+(?:[^.]{0,40}?)(나와|명시|적혀|없|비어|갱신)/, why: '독자에게 내부 데이터를 지칭했다. «공고» 로 바꿔라' },
  ...
  // 어미만 바꿔 규칙을 피해 간 사례가 있었다(다르다 -> 다르므로 -> 다릅니다).
  // 문자열이 아니라 패턴으로 막는다.
  // 한글은 완성형이라 «다릅니다» 안에 «다르» 가 들어 있지 않다(르 != 릅).
  // 처음 쓴 /다르/ 는 «다릅니다» 를 통과시켰다. 종성이 붙는 음절까지 함께 적는다.
  { re: /(시기|기간)[은는]\s*접수기관마다\s*다[르릅]/, why: '여러 문서에서 반복된 상투구다. 어미를 바꾸는 것으로는 해결되지 않는다. 그 제도에서 중요한 것부터 쓰도록 문장을 다시 짜라' },
  // «확인하는 것이 정확합니다» 를 막았더니 좋습니다/안전합니다/필요합니다 로 갈아탔다. 어미를 열어 둔다.
  { re: /확인하는 것이 (정확|좋|안전|필요)/, why: '여러 문서에서 반복된 맺음 상투구다. 어미를 바꾸는 것으로는 해결되지 않는다' },
  // 26건 중 11건이 연락처를 이 한 문장으로 끝냈다. 연락처는 사실이지만 틀이 같으면 반복으로 잡힌다.
  { re: /문의는 .{0,60}하면 됩니다/, why: '여러 문서에서 반복된 연락처 상투구다. 그 기관이 무엇을 맡는지로 문장을 바꿔라' },
  // 26건 중 14건에 들어 있었다. 공고를 인용할 때 기본값처럼 쓰이는 틀이다.
  { re: /안내(되어|돼) (있|잇)/, why: '여러 문서에서 반복된 인용 상투구다. 공고가 무엇을 정했는지 직접 써라' },
];
